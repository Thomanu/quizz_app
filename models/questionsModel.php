...
class questions extends database
{
    ...
    theme.`description`
    FROM
        ' . $this->tablename . ' AS question
        INNER JOIN `themes` AS theme
        ON
    theme.`theme_id` = question.`theme_id` ORDER BY RAND()';
    ...
    public function getQuestionbyId()
    {
        ...
        FROM
            ...
            INNER JOIN `themes` AS theme
            ON
        theme.`theme_id` = question.`theme_id` WHERE id = :id';
    ...
    public function getQuestionByCat()
    {
        $query = 'SELECT
    `question`.`id`,
    `question`.`question_category`,
...
INNER JOIN `themes` AS theme
ON
    theme.`theme_id` = question.`theme_id` WHERE `question_cat_id` = :question_cat_id';
        $getQuestionByCat = $this->db->prepare($query);
        $getQuestionByCat->bindValue(':question_cat_id', $this->question_cat_id, PDO::PARAM_INT);
        $getQuestionByCat->execute();
        if ($getQuestionByCat->execute()) {
            $question = $getQuestionByCat->fetchAll(PDO::FETCH_OBJ);
        }
        return $question;
    }
...
INNER JOIN `themes` AS theme
ON
    theme.`theme_id` = question.`theme_id` WHERE `theme_id` = :theme_id';
        $getQuestionByTheme = $this->db->prepare($query);
        $getQuestionByTheme->bindValue(':theme_id', $this->theme_id, PDO::PARAM_INT);
        $getQuestionByTheme->execute();
        if ($getQuestionByTheme->execute()) {
            $question = $getQuestionByTheme->fetchAll(PDO::FETCH_OBJ);
        }
        return $question;
    }
    ...
    public function updateQuestionType()
    {
        ...
        $updateQuestionType->bindValue(':question_type', $this->question_type, PDO::PARAM_STR);
